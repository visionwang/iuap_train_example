...
gulp.task("install", ["package"], function(){
  ...
  // 安装命令
  var installCommandStr = `${command} install:install-file -Dfile=${targetPath}/${PACKAGE_WAR_NAME} -DgroupId=com.yonyou.iuap.pap -DartifactId=currtype -Dversion=0.0.1-SNAPSHOT -Dpackaging=war`;
...
gulp.task("deploy", ["install"], function(){
  ...
  var publishCommandStr =  `${command} deploy:deploy-file  -Dfile=${targetPath}/${PACKAGE_WAR_NAME} -DgroupId=com.yonyou.iuap.pap -DartifactId=currtype -Dversion=0.0.1-SNAPSHOT -Dpackaging=war -DrepositoryId=${repositoryId} -Durl=${repositoryURL}`;
