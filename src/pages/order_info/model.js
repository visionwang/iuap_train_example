...
export default {
    ...
    effects: {
            ...
            if (res) {
                if(res.content&&res.content.length){
                    ...
                    for(let i=0;i<res.content.length;i++){
                        ...
                        res.content[i].orderType = selectArray[Number(res.content[i].orderType)];
                        res.content[i].isPaid = temp.isPaid ? 0 :1;
                        res.content[i].orderNo = temp.orderNo+"";
                        res.content[i].purOrg = temp.purOrg+"";
                        res.content[i].releaseTime = temp.releaseTime+"";
                        res.content[i].orderAmount = temp.orderAmount || 0.00;
        ...
        async save(param,getState){//保存
            ...
            let res=processData(await api.saveOrderInfo(param));
